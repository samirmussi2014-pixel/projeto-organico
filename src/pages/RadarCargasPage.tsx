...
import {
  ESTADOS_BR, SEGMENTOS, TIPOS_CARGA, TIPOS_VEICULO,
  CLASSIFICACOES, classificacaoColor, statusColor, formatDate,
} from '@/lib/constants';
import { autoScoreFromFields } from '@/lib/scoring';
import { supabase } from '@/lib/supabase';

const HIGH_VALUE_KEYS = ['empresa', 'contato_publico', 'telefone_publico', 'email_publico', 'evidencia', 'rota', 'frequencia', 'url_fonte'];

const EMPTY: Partial<Carga> = {
  empresa: '', cidade: '', estado: '', segmento: '', tipo_carga: '',
  origem: '', destino: '', rota: '', tipo_veiculo: '', capacidade: '',
  frequencia: '', contato_publico: '', telefone_publico: '', email_publico: '',
  url_fonte: '', nome_fonte: '', evidencia: '', observacoes: '',
  score: 0, classificacao: 'NAO CONFIRMADA', status: 'NOVO',
};

export function RadarCargasPage() {
  const { data, loading, create, update, remove } = useCrud<Carga>('cargas');
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<Carga | null>(null);
  const [form, setForm] = useState<Partial<Carga>>(EMPTY);
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      rows = rows.filter((r) =>
        [r.empresa, r.cidade, r.segmento, r.tipo_carga, r.origem, r.destino, r.nome_fonte]
  ...
  async function convertToOportunidade(c: Carga) {
    await supabase.from('oportunidades').insert({
      ...
      fonte: c.nome_fonte,
      acao_recomendada: 'Verificar disponibilidade e entrar em contato',
      status: 'NOVO',
      origem_tipo: 'cargas',
      origem_id: c.id,
    });
  ...
  return (
    <div className="space-y-4">
      ...
      <SectionCard title="Cargas Identificadas">
        {loading ? (
          ...
          <EmptyState
            ...
            description="Adicione cargas identificadas em fontes públicas."
            action={
              <Button onClick={openNew} size="sm">
          ...
          <DataTable
            columns={[
              ...
              { key: 'rota', label: 'Rota', render: (r) => r.rota || `${r.origem || '...'} → ${r.destino || '...'}` },
              ...
              { key: 'data_descoberta', label: 'Descoberta', render: (r) => formatDate(r.data_descoberta) },
              {
                key: 'actions',
                label: '',
                render: (r) => (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar Carga' : 'Nova Carga'} size="xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Field label="Telefone Público"><Input value={form.telefone_publico ?? ''} onChange={(e) => setForm({ ...form, telefone_publico: e.target.value })} /></Field>
            <Field label="E-mail Público"><Input value={form.email_publico ?? ''} onChange={(e) => setForm({ ...form, email_publico: e.target.value })} /></Field>
            <Field label="Nome da Fonte"><Input value={form.nome_fonte ?? ''} onChange={(e) => setForm({ ...form, nome_fonte: e.target.value })} /></Field>
            <Field label="URL da Fonte"><Input value={form.url_fonte ?? ''} onChange={(e) => setForm({ ...form, url_fonte: e.target.value })} /></Field>
            <Field label="Data da Descoberta"><Input type="date" value={form.data_descoberta ?? ''} onChange={(e) => setForm({ ...form, data_descoberta: e.target.value })} /></Field>
          </div>
